...
function getIndicators() {
  axios.get("https://localhost:5001/api/Indicator").then((result) => {
    console.log(result);
...
function tryNewIndicator(name, value, unit, x, y) {
  ...
  axios
    .post("https://localhost:5001/api/Indicator", {
      name: name,
      description: name,
      x,
      y,
...
function changeBackgroundImage(id) {
  ...
  element.src = "https://localhost:5001/api/BackgroundImage/" + id;
  console.log(element);
}
...
document
  ...
  .addEventListener("submit", function (event) {
    ...
    if (fileInput) {
      ...
      axios
        .post(
          "https://localhost:5001/api/backgroundimage/upload-image",
          formData
        )
        .then((result) => {
...
function onDelete(e, id) {
  axios.delete(`https://localhost:5001/api/indicator/${id}`);
